...
export default class CameraService {
  ...
  /**
   * Finds objects in the given scene having the specified type.
   * 
   * @param {THREE.Scene} scene - scene to search
   * @param {String} type - object type
   * @returns {THREE.Object3D[]} list of objects having the given type
   */
  static getObjectsByType = (scene, type) => {
    const findObjects = (o) => {
      let children = [];
      ...
      o.children.forEach((child) => {
        ...
        if (Array.isArray(child.children)) {
          children = children.concat(findObjects(child));
        }
      });
      return children;
    }
    return findObjects(scene);
  }

  /**
   * Calculates the sprite scale based on distance to camera.
   * 
   * @param {THREE.Sprite} sprite - sprite to scale
   * @param {THREE.Vector3} cameraWorld - world position of camera
   * @returns {Number} sprite scale
   */
  static getSpriteScale = (sprite, cameraWorld) => {
    const scaleVector = new Vector3();
    const spriteWorld = CameraService.getWorldPosition(sprite);
    const minDistance = Constants.WebGL.Camera.SATELLITE_LABEL_RANGE;
    const magnitude = scaleVector
      .subVectors(spriteWorld, cameraWorld)
      .length();

    if (sprite.isSatellite && magnitude > minDistance) {
      return 0;
    }
